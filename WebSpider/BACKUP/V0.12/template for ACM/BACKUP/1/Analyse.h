...
void getURLs(string &content, queue<URL>&q, int deep){
	if (finished + q.size() > MAXOFWEBS)return;

	const static char href[] = "a href=\"http"; int cnt = 0, size = content.size(); string url = "http";
	for (int i = 0; i < size; ++i){
		if (content[i] == href[cnt])++cnt;
		else cnt = 0;

		if (cnt == 12){
			for (int j = i + 1; url.size() < 100 && j < content.size() && content[j] != '"' && content[j] != '\n'; ++j, ++i)url += content[j];
